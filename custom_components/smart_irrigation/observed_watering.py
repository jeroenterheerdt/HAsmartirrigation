...
class ObservedWateringMixin:
    ...
    async def _credit_from_volume(
        ...
        ``seconds`` is how long the valve was open for that volume, and
        ``started`` when it opened. Neither plays any part in the credit, since
        the meter already knows what came out, but the first lets the flow
        calibration check the configured throughput and the second dates the run
        in the irrigation history.
        """
        if volume_l <= 0:
            return
        zone = self.store.get_zone(zone_id)
