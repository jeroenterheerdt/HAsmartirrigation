...
class RecurringScheduleManager:
    ...
    async def async_create_schedule(self, schedule_data: Dict[str, Any]) -> None:
        ...
        _LOGGER.info("Created recurring schedule: %s", schedule_data[const.SCHEDULE_CONF_NAME])

    async def async_update_schedule(self, schedule_id: str, schedule_data: Dict[str, Any]) -> None:
        ...
        _LOGGER.info("Updated recurring schedule: %s", schedule_data.get(const.SCHEDULE_CONF_NAME, schedule_id))
    ...
    async def async_delete_schedule(self, schedule_id: str) -> None:
        ...
        self._schedules = [s for s in self._schedules if s[const.SCHEDULE_CONF_ID] != schedule_id]
    ...
    async def _setup_weekly_tracker(self, schedule: Dict[str, Any]) -> Any:
        ...
        def check_and_execute(now):
            ...
            if any(day_mapping.get(day_name) == current_weekday for day_name in day_names):
    ...
    @callback
    def _execute_schedule(self, schedule: Dict[str, Any], now: datetime.datetime) -> None:
        ...
        _LOGGER.info("Executing recurring schedule: %s (action: %s)", schedule_name, action)
        ...
        self.hass.async_create_task(self._perform_schedule_action(action, zones, schedule_name))

    async def _perform_schedule_action(self, action: str, zones: Union[str, List[str]], schedule_name: str) -> None:
        ...
        try:
            if action == "calculate":
                ...
                self.hass.bus.fire(f"{const.DOMAIN}_{const.EVENT_IRRIGATE_START}", event_data)
            
            _LOGGER.info("Successfully executed schedule action: %s for zones: %s", action, zones)
...
class SeasonalAdjustmentManager:
    ...
    async def async_create_adjustment(self, adjustment_data: Dict[str, Any]) -> None:
        ...
        _LOGGER.info("Created seasonal adjustment: %s", adjustment_data[const.SEASONAL_CONF_NAME])

    async def async_update_adjustment(self, adjustment_id: str, adjustment_data: Dict[str, Any]) -> None:
        ...
        _LOGGER.info("Updated seasonal adjustment: %s", adjustment_data.get(const.SEASONAL_CONF_NAME, adjustment_id))

    async def async_delete_adjustment(self, adjustment_id: str) -> None:
        """Delete a seasonal adjustment."""
        # Remove from adjustments list
        self._adjustments = [a for a in self._adjustments if a[const.SEASONAL_CONF_ID] != adjustment_id]
    ...
    async def apply_seasonal_adjustments(self, zone_data: Dict[str, Any], zone_id: Optional[int] = None) -> Dict[str, Any]:
        ...
        for adjustment in self._adjustments:
            ...
            multiplier_adj = adjustment.get(const.SEASONAL_CONF_MULTIPLIER_ADJUSTMENT, 1.0)
            ...
            threshold_adj = adjustment.get(const.SEASONAL_CONF_THRESHOLD_ADJUSTMENT, 0.0)
        ...
        if applied_adjustments:
            ...
            _LOGGER.info("Applied %d seasonal adjustments to zone %s", len(applied_adjustments), zone_id)
