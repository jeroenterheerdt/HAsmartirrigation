...
class IrrigationUnlimitedIntegration:
    ...
    async def async_initialize(self) -> None:
        ...
        self._sync_enabled = config.get(const.CONF_IRRIGATION_UNLIMITED_INTEGRATION, False)
        self._entity_prefix = config.get(const.CONF_IU_ENTITY_PREFIX, const.CONF_DEFAULT_IU_ENTITY_PREFIX)
        
        if self._sync_enabled:
            await self._discover_iu_entities()
            _LOGGER.info("Irrigation Unlimited integration enabled with %d entities", len(self._iu_entities))
    ...
    async def _trigger_iu_entity(self, entity_id: str, duration: int) -> None:
        ...
        try:
            ...
            _LOGGER.debug("Triggered IU entity %s for %d seconds using manual_run", entity_id, duration)
            return
        except Exception as e:
    ...
    async def async_send_zone_data_to_iu(self, zone_id: int, data: Dict[str, Any]) -> bool:
        ...
        iu_entity = await self._find_matching_iu_entity(zone)
        if not iu_entity:
            _LOGGER.warning("No matching IU entity found for zone %s", zone.get(const.ZONE_NAME))
            return False
    ...
    async def async_create_iu_schedule_from_smart_irrigation(self, zone_ids: Optional[List[int]] = None) -> Dict[str, Any]:
        ...
        for schedule in recurring_schedules:
            ...
            try:
                iu_schedule = await self._convert_recurring_schedule_to_iu_schedule(schedule)
                if iu_schedule:
                    schedule_data["created_schedules"].append({
    ...
    async def _convert_trigger_to_iu_schedule(self, trigger: Dict[str, Any]) -> Optional[Dict[str, Any]]:
        ...
        if trigger_type == const.TRIGGER_TYPE_SUNRISE:
            ...
            iu_schedule.update({
                ...
                "offset": f"{offset_minutes:+d} minutes" if offset_minutes != 0 else "0"
            })
    ...
    async def _convert_recurring_schedule_to_iu_schedule(self, schedule: Dict[str, Any]) -> Optional[Dict[str, Any]]:
        ...
        schedule_name = schedule.get(const.SCHEDULE_CONF_NAME, "Smart Irrigation Schedule")
    ...
    async def async_update_configuration(self, config_data: Dict[str, Any]) -> None:
        ...
        self._sync_enabled = config_data.get(const.CONF_IRRIGATION_UNLIMITED_INTEGRATION, False)
        self._entity_prefix = config_data.get(const.CONF_IU_ENTITY_PREFIX, const.CONF_DEFAULT_IU_ENTITY_PREFIX)
